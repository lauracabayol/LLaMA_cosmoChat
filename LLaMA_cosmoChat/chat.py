...
class ChatCosmoHub:
    ...
    def _print_system_specs(self) -> None:
        ...
        if is_cuda_available:
            for i in range(num_cuda_devices):
                ...
                logger.info(f"Total Memory: {torch.cuda.get_device_properties(i).total_memory} bytes")
    ...
    def _extract_sql(self, content: str) -> str:
        ...
        Raises:
        - ValueError: If SQL query keyword is not found in content
        """
        ...
        if sql_pos == -1:
            raise ValueError("Could not find SQL query in generated content")

        sql_query = content.split(content[sql_pos:sql_pos + len(sql_keyword)])[1].strip()
    ...
    def query_LLaMA(self, query: str) -> Tuple[str, str]:
        ...
        print(content)
        sql_part, python_plot = self._extract_sql(content)
        
        return sql_part, python_plot

    def query_cosmohub(self, query: str) -> pd.DataFrame:
        """
